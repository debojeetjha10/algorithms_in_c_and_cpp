...
vector<int> getPathBFS(int**edges, int n, int starting_vertex, int end_vertex, bool*visited, vi v)
{
	...
	if(flag)
	{
	    ...
	    while(end_vertex!=starting_vertex)
	    {
		    v.PB(end_vertex);
		    end_vertex=arr[end_vertex];
	    }
	    v.PB(starting_vertex);
	    return v;
	}
...
void solve()
{
	...
	vi v;
	v=getPathBFS(edges,n,starting_vertex,end_vertex,visited,v);
	for(auto x: v)
		cout<<x<<" ";
