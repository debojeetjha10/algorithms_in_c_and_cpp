...
void has_path_DFS(int**edges, int n, int starting_vertex, bool*visited)
{
	...
	for(int i=0; i<n; i++)
	{
		...
		if(edges[starting_vertex][i]==1)
		{
			...
			has_path_DFS(edges,n,i,visited);
		}
	}

}

bool DFS(int**edges, int n,int starting_vertex,int end_vertex)
{
	...
	has_path_DFS(edges,n,starting_vertex,visited);
...
void solve()
{
	...
	bool ans=DFS(edges,n,starting_vertex,end_vertex);
	if(ans)
		cout<<"true"<<endl;
	else
		cout<<"false"<<endl;
